...
class BaseTrainer(ABC):
    ...
    def reset(self) -> None:
        """Initialize or reset the instance to yield a new iterator from
        zero."""
        self.is_run = False
        self.env_step = 0
    ...
    def __next__(
            self) -> Union[None, Tuple[int, Dict[str, Any], Dict[str, Any]]]:
        ...
        if not self.stop_fn_flag:
            self.logger.save_data(self.epoch, self.env_step,
                                  self.gradient_step, self.save_checkpoint_fn)
            # test
            if self.test_collector is not None:
    ...
    def log_update_data(self, data: Dict[str, Any], losses: Dict[str,
                                                                 Any]) -> None:
    ...
    @abstractmethod
    def policy_update_fn(self, data: Dict[str, Any],
