...
class Collector(object):
    ...
    def reset(
        ...
        """Reset the environment, statistics, current data and possibly replay
        memory.
    ...
    def _reset_hidden_state(self, identifier: Union[int, List[int]]) -> None:
        ...
        if hasattr(self.data.policy, 'hidden_state'):
            ...
            if isinstance(state, torch.Tensor):
                ...
                state[identifier] = None if state.dtype == object else 0
            elif isinstance(state, Batch):
                state.empty_(identifier)
    ...
    def _reset_env_with_ids(
        ...
        if self.preprocess_fn:
            processed_data = self.preprocess_fn(obs=obs_reset,
                                                info=info,
                                                env_id=global_ids)
...
class AsyncCollector(Collector):
    ...
    def collect(
        ...
        """Collect a specified number of step or episode with async env
        setting.
