...
class ContinuousToDiscrete(gym.ActionWrapper):
    """Gym environment wrapper to take discrete action in a continuous
    environment.
    ...
    def __init__(self, env: gym.Env, action_per_dim: int | list[int]) -> None:
        ...
        self.mesh = np.array(
            [
                np.linspace(lo, hi, a)
                for lo, hi, a in zip(low, high, action_per_dim, strict=True)
            ],
            dtype=object,
        )
    ...
    def action(self, act: np.ndarray) -> np.ndarray:  # type: ignore
        # modify act
        assert len(
            act.shape) <= 2, f'Unknown action format with shape {act.shape}.'
        if len(act.shape) == 1:
            return np.array([self.mesh[i][a] for i, a in enumerate(act)])
        return np.array([[self.mesh[i][a] for i, a in enumerate(a_)]
                         for a_ in act])


class MultiDiscreteToDiscrete(gym.ActionWrapper):
    """Gym environment wrapper to take discrete action in multidiscrete
    environment.
