...
class BaseVectorEnv:
    ...
    generator ``env_fn`` of a specific task means that ``env_fn()`` returns the
    environment of the given task, for example, ``gym.make(task)``.

    All of the VectorEnv must inherit :class:`~rltoolkit.envs.BaseVectorEnv`.
    ...
    def reset(
        ...
        **kwargs: Any,
        ...
        assert (isinstance(ret_list[0], (tuple, list))
                and len(ret_list[0]) == 2 and isinstance(ret_list[0][1], dict))
...
class DummyVectorEnv(BaseVectorEnv):
    ...
    .. seealso::
        ...
        Please refer to :class:`~rltoolkit.env.BaseVectorEnv` for other APIs' usage.
    """
...
class RayVectorEnv(BaseVectorEnv):
    ...
    def __init__(
        ...
        if not ray.is_initialized():
            ray.init()
        super().__init__(env_fns, lambda env_fn: RayEnvWorker(env_fn),
                         wait_num, timeout)
