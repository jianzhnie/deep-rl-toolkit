...
class EnvWorker(ABC):
    ...
    def send(self, action: Optional[np.ndarray]) -> None:
        ...
        When action is None, it indicates sending "reset" signal; otherwise it
        indicates "step" signal. The paired return value from "recv" function
        is determined by such kind of different signal.
        """
        if hasattr(self, 'send_action'):
            deprecation('send_action will soon be deprecated. '
    ...
    def recv(
        ...
        info) or (obs, rew, terminated, truncated, info), based on whether the
        environment is using the old step API or the new one.
        """
        if hasattr(self, 'get_result'):
            deprecation('get_result will soon be deprecated. '
                        'Please use send and recv for your own EnvWorker.')
    ...
    def step(self, action: np.ndarray) -> gym_new_venv_step_type:
        ...
        "step" function. But they can be called separately in async simulation,
        i.e. someone calls "send" first, and calls "recv" later.
        """
        self.send(action)
