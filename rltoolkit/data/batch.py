...
def _to_array_with_correct_type(obj: Any) -> np.ndarray:
    if isinstance(obj, np.ndarray) and issubclass(obj.dtype.type,
                                                  (np.bool_, np.number)):
    ...
    if obj_array.dtype == object:
        ...
        if not obj_array.shape:
            ...
            return obj_array  # various length, np.array([[1], [2, 3], [4, 5, 6]])
                ...
                for arr in obj_array.reshape(-1)):
...
def _assert_type_keys(keys: Iterable[str]) -> None:
    assert all(isinstance(key, str)
               for key in keys), f'keys should all be string, but got {keys}'


def _parse_value(
        obj: Any) -> Optional[Union['Batch', np.ndarray, torch.Tensor]]:
    if isinstance(obj, Batch):  # most often case
        return obj
    elif ((isinstance(obj, np.ndarray) and issubclass(obj.dtype.type,
                                                      (np.bool_, np.number)))
          or isinstance(obj, torch.Tensor) or obj is None):  # third often case
        return obj
        ...
        if (not isinstance(obj, np.ndarray) and isinstance(obj, Collection)
                and len(obj) > 0
            ...
            try:
                ...
                raise TypeError(
                    ...
                    ' of torch.Tensor as unique value yet.') from exception
        if _is_batch_set(obj):
            ...
            try:
                ...
                raise TypeError(
                    ...
                    'tuple of tensors as unique value yet.') from exception
        return obj


def _alloc_by_keys_diff(meta: 'Batch',
                        batch: 'Batch',
                        size: int,
...
class Batch:
    ...
    def __init__(
        self,
        batch_dict: Optional[Union[dict, 'Batch', Sequence[Union[dict,
                                                                 'Batch']],
                                   np.ndarray]] = None,
        copy: bool = False,
        **kwargs: Any,
    ...
    def __getattr__(self, key: str) -> Any:
        ...
        The "Any" return type is needed for mypy.
        """
    ...
    def __iadd__(self, other: Union['Batch', Number, np.number]) -> 'Batch':
        ...
        if isinstance(other, Batch):
            for (batch_key, obj), value in zip(
                    self.__dict__.items(),
                    other.__dict__.values()):  # TODO are keys consistent?
                if isinstance(obj, Batch) and obj.is_empty():
                    continue
                else:
    ...
    def __imul__(self, value: Union[Number, np.number]) -> 'Batch':
        ...
        assert _is_number(
            value), 'Only multiplication by a number is supported.'
    ...
    def __cat(self, batches: Sequence[Union[dict, 'Batch']],
              lens: List[int]) -> None:
        ...
        keys_map = [
            set(batch_key for batch_key, obj in batch.items()
                if not (isinstance(obj, Batch) and obj.is_empty()))
            for batch in batches
        ]
        keys_shared = set.intersection(*keys_map)
        values_shared = [[batch[key] for batch in batches]
                         for key in keys_shared]
        for key, shared_value in zip(keys_shared, values_shared):
            if all(
                    isinstance(element, (dict, Batch))
                    for element in shared_value):
                batch_holder = Batch()
        ...
        for key in keys_partial:
            for i, batch in enumerate(batches):
                ...
                try:
                    ...
                    self.__dict__[key] = _create_value(value,
                                                       sum_lens[-1],
                                                       stack=False)
                    self.__dict__[key][sum_lens[i]:sum_lens[i + 1]] = value

    def cat_(self, batches: Union['Batch', Sequence[Union[dict,
                                                          'Batch']]]) -> None:
        ...
        for batch in batches:
            if isinstance(batch, dict):
                ...
                raise ValueError(
                    f'Cannot concatenate {type(batch)} in Batch.cat_')
        if len(batch_list) == 0:
            return
        batches = batch_list
        try:
            ...
            # keep it.
            lens = [
                0 if batch.is_empty(recurse=True) else len(batch)
                for batch in batches
            ]
            ...
            raise ValueError(
                ...
                'concatenation of scalar.') from exception
    ...
    def stack_(self,
               batches: Sequence[Union[dict, 'Batch']],
               axis: int = 0) -> None:
        ...
        for batch in batches:
            if isinstance(batch, dict):
                ...
                raise ValueError(
                    f'Cannot concatenate {type(batch)} in Batch.stack_')
        if len(batch_list) == 0:
            return
        batches = batch_list
        if not self.is_empty():
        ...
        for key in keys_partial:
            for i, batch in enumerate(batches):
                ...
                value = batch.get(key)
                if isinstance(value,
                              Batch) and value.is_empty():  # type: ignore
    ...
    @staticmethod
    def stack(batches: Sequence[Union[dict, 'Batch']],
              axis: int = 0) -> 'Batch':
    ...
    def empty_(self,
               index: Optional[Union[slice, IndexType]] = None) -> 'Batch':
    ...
    def update(self,
               batch: Optional[Union[dict, 'Batch']] = None,
               **kwargs: Any) -> None:
    ...
    def __len__(self) -> int:
        ...
        for obj in self.__dict__.values():
            if isinstance(obj, Batch) and obj.is_empty(recurse=True):
                continue
            elif hasattr(obj, '__len__') and (isinstance(obj, Batch)
                                              or obj.ndim > 0):
                lens.append(len(obj))
            else:
        ...
        if self.is_empty():
            ...
            for obj in self.__dict__.values():
                try:
                    ...
                    data_shape.append([])
            return (list(map(min, zip(
                *data_shape))) if len(data_shape) > 1 else data_shape[0])
