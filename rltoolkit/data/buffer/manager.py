...
class ReplayBufferManager(ReplayBuffer):
    ...
    def sample_indices(self, batch_size: int) -> np.ndarray:
        ...
        if batch_size == 0:  # get all available indices
            ...
            buffer_idx = np.random.choice(self.buffer_num,
                                          batch_size,
                                          p=self._lengths /
                                          self._lengths.sum())
...
class PrioritizedReplayBufferManager(PrioritizedReplayBuffer,
                                     ReplayBufferManager):
    """PrioritizedReplayBufferManager contains a list of
    PrioritizedReplayBuffer with exactly the same configuration.
...
class HERReplayBufferManager(ReplayBufferManager):
    """HERReplayBufferManager contains a list of HERReplayBuffer with exactly
...
def _prev_index(
    ...
    for start, end, cur_len, last in zip(offset[:-1], offset[1:], lengths,
                                         last_index):
...
def _next_index(
    ...
    for start, end, cur_len, last in zip(offset[:-1], offset[1:], lengths,
        ...
        if np.sum(mask) > 0:
            ...
            next_index[mask] = (subind - start + 1 -
                                end_flag) % cur_len + start
    return next_index
