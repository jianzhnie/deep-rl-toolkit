...
class ReplayBuffer:
    """:class:`~rltoolkit.data.ReplayBuffer` stores data generated from interaction \
    ...
    def __setattr__(self, key: str, value: Any) -> None:
        ...
        assert key not in self._reserved_keys, (
            "key '%s' is reserved and cannot be assigned" % key)
    ...
    def load_hdf5(cls,
                  path: str,
                  device: Optional[str] = None) -> 'ReplayBuffer':
    ...
    def _add_index(
            self, rew: Union[float, np.ndarray],
    ...
    def __getitem__(self, index: Union[slice, int, List[int],
        ...
        if isinstance(index, slice):  # change slice to np array
            ...
            indices = (self.sample_indices(0) if index == slice(None) else
                       self._indices[:len(self)][index])
        else:
            indices = index  # type: ignore
