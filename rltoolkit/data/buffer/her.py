...
class HERReplayBuffer(ReplayBuffer):
    ...
    def _restore_cache(self) -> None:
        ...
        It's called everytime before 'writing', 'sampling' or 'saving' the
        buffer.
        """
    ...
    def update(self, buffer: Union['HERReplayBuffer',
                                   'ReplayBuffer']) -> np.ndarray:
        self._restore_cache()
    ...
    def rewrite_transitions(self, indices: np.ndarray) -> None:
        """Re-write the goal of some sampled transitions' episodes according to
        HER.

        Currently applies only HER's 'future' strategy. The new goals will be
        written directly to the internal batch data temporarily and will be
        restored right before the next sampling or when using some of the
        buffer's method (e.g. `add`, `save_hdf5`, etc.). This is to make sure
        that n-step returns calculation etc., performs correctly without
        ...
        #   presented episodes
        unique_ep_open_indices = np.sort(
            np.unique(terminal, return_index=True)[1])
        unique_ep_indices = indices[:, unique_ep_open_indices]
        #   close indices are used to find max future_t among presented episodes
        unique_ep_close_indices = np.hstack([(unique_ep_open_indices - 1)[1:],
                                             len(terminal) - 1])
        ...
        ep_obs.desired_goal[:, her_ep_indices] = future_obs.achieved_goal[
            None, her_ep_indices]
        if self._save_obs_next:
            ep_obs_next.desired_goal[:,
                                     her_ep_indices] = future_obs.achieved_goal[
                                         None, her_ep_indices]
            ep_rew[:, her_ep_indices] = self._compute_reward(
                ep_obs_next)[:, her_ep_indices]
        else:
            tmp_ep_obs_next = self[self.next(unique_ep_indices)].obs
            ep_rew[:, her_ep_indices] = self._compute_reward(
                tmp_ep_obs_next)[:, her_ep_indices]
    ...
    def _compute_reward(self, obs: Batch, lead_dims: int = 2) -> np.ndarray:
        ...
        ag = obs.achieved_goal.reshape(-1,
                                       *obs.achieved_goal.shape[lead_dims:])
