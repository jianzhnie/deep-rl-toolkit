...
class A2CPolicy(PGPolicy):
    ...
    def _compute_returns(self, batch: Batch, buffer: ReplayBuffer,
        ...
        if self.reward_normalization:  # unnormalize v_s & v_s_
        ...
        if self.reward_normalization:
            batch.returns = unnormalized_returns / np.sqrt(self.ret_rms.var +
                                                           self._eps)
    ...
    def learn(  # type: ignore
        ...
        for _ in range(repeat):
            for minibatch in batch.split(batch_size, merge_last=True):
                ...
                log_prob = log_prob.reshape(len(minibatch.adv),
                                            -1).transpose(0, 1)
