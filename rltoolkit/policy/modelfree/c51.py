...
class C51Policy(DQNPolicy):
    ...
    def _target_dist(self, batch: Batch) -> torch.Tensor:
        if self._target:
            act = self.forward(batch, input='obs_next').act
            next_dist = self.forward(batch,
                                     model='target_model',
                                     input='obs_next').logits
        else:
            next_batch = self.forward(batch, input='obs_next')
            act = next_batch.act
        ...
        target_dist = (
            1 -
            (target_support.unsqueeze(1) - self.support.view(1, -1, 1)).abs() /
            self.delta_z).clamp(0, 1) * next_dist.unsqueeze(1)
