...
class MLP(nn.Module):
    ...
    * hidden_sizes[-1] * output_dim
    ...
    def __init__(
        ...
        activation: Optional[Union[ModuleType,
                                   Sequence[ModuleType]]] = nn.ReLU,
        ...
        if norm_layer:
            if isinstance(norm_layer, list):
                ...
                if isinstance(norm_args, list):
                    ...
                    norm_args_list = [
                        norm_args for _ in range(len(hidden_sizes))
                    ]
            else:
                norm_layer_list = [
        ...
        if activation:
            if isinstance(activation, list):
                ...
                if isinstance(act_args, list):
                    ...
                    act_args_list = [
                        act_args for _ in range(len(hidden_sizes))
                    ]
            else:
                activation_list = [
        ...
        for in_dim, out_dim, norm, norm_args, activ, act_args in zip(
            ...
            model += miniblock(in_dim, out_dim, norm, norm_args, activ,
                               act_args, linear_layer)
        if output_dim > 0:
...
class EnsembleLinear(nn.Module):
    ...
    def __init__(
        ...
        weight_data = torch.rand(
            (ensemble_size, in_feature, out_feature)) * 2 * k - k
...
class BranchingNet(nn.Module):
    ...
    def forward(
        ...
        action_scores = action_scores - torch.mean(
            action_scores, 2, keepdim=True)
        logits = value_out + action_scores
        return logits, state


def get_dict_state_decorator(state_shape: Dict[str, Union[int, Sequence[int]]],
    ...
    def preprocess_obs(
        obs: Union[Batch, dict, torch.Tensor, np.ndarray], ) -> torch.Tensor:
        if isinstance(obs, dict) or (isinstance(obs, Batch)
                                     and keys[0] in obs):
            if original_shape[keys[0]] == obs[keys[0]].shape:
                ...
                new_obs = torch.cat(
                    [torch.Tensor(obs[k].reshape(bsz, -1)) for k in keys],
                    dim=1)
        else:
