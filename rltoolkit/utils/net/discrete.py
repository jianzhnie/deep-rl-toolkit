...
class ImplicitQuantileNetwork(Critic):
    ...
    def forward(  # type: ignore
            self, obs: Union[np.ndarray, torch.Tensor], sample_size: int,
        ...
        out = self.last(embedding).view(batch_size, sample_size,
                                        -1).transpose(1, 2)
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        phi2_hat = self.forward_model(
            torch.cat([phi1, F.one_hot(act, num_classes=self.action_dim)],
                      dim=1))
