...
def convert_tfevents_to_csv(root_dir, refresh=False):
    """Recursively convert test/reward from all tfevent file under root_dir to
    csv.
    ...
    print(
        f'Converting {len(tfevent_files)} tfevents files under {root_dir} ...')
    result = {}
...
def merge_csv(csv_files, root_dir, remove_zero=False):
    ...
    content = [['env_step', 'reward', 'reward:shaded'] + list(
        map(lambda f: 'reward:' + os.path.relpath(f, root_dir), sorted_keys))]
    ...
    output_path = os.path.join(root_dir,
                               f'test_reward_{len(csv_files)}seeds.csv')
    print(
        f'Output merged csv file to {output_path} with {len(content[1:])} lines.'
    )
