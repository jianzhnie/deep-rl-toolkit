...
def get_args():
    ...
    parser.add_argument('--dueling-q-hidden-sizes',
                        type=int,
                        nargs='*',
                        default=[128, 128])
    parser.add_argument('--dueling-v-hidden-sizes',
                        type=int,
                        nargs='*',
                        default=[128, 128])
...
def test_dqn(args=get_args()):
    ...
    def train_fn(epoch, env_step):
        if env_step <= 100000:
            ...
            eps = args.eps_train - (env_step -
                                    100000) / 400000 * (0.5 * args.eps_train)
            policy.set_eps(eps)
        else:
    ...
    if __name__ == '__main__':
        ...
        result = test_collector.collect(n_episode=args.test_num,
                                        render=args.render)
        rews, lens = result['episode_reward'], result['episode_length']
